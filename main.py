    ...
    try:
        ...
        if '/banuser' in msgText:
            ...
            if isadmin:
                ...
                bot.sendMes(update.message.chat.id,'❌No Tiene Permiso❌')
            return
            
        if '/getdb' in msgText:
            isadmin = jdb.is_admin(username)
            if isadmin:
                bot.sendMessage(update.message.chat.id,'🗑️Base De Datos📔')
        ...
        if '/view_proxy' in msgText:
                try:
                    ...
                    if getUser:
                        ...
                        else : bot.sendMessage(update.message.chat.id,'🌀No tienes ningún proxy🌀')
                except:
                    if user_info:
        ...
        if '/off_proxy' in msgText:
            try:
                ...
                if getUser:
                    ...
                    bot.sendMessage(update.message.chat.id,'☑️Proxy desactivado☑️')
            except:
                if user_info:
                    user_info['proxy'] = ''
        ...
        if '/history_changes' in msgText:
            history = open('history_changes.txt','r')
            bot.sendMessage(update.message.chat.id,history.read())
            return
        ...
        if '/start' in msgText:
            start_i = start.start_i(username)    
            bot.sendMessage(update.message.chat.id,start_i)       
